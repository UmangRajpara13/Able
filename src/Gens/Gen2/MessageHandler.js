...
var  sttpid, wsMap = new Map(), awareness = {}
...
var interrogativeWords = [
  ...
  // read actions 
const globalWatcher = watch(join(process.cwd(), 'able_store/Gen2'))
  ...
  try {
    if (path.endsWith('.json')) {
      readJson(path, (err, file) => {
        ...
        if (path == join(process.cwd(), 'able_store/all/Gen2/global.json')) {
...
export function MessageHandlerGen2(recievedData,ws) {
        ...
        case `stt`:
            ...
            if (interrogativeWords.some((startString) => query.startsWith(startString)) |
                ...
                if (globalActionsKeys.includes(action)) {
                    ...
                    ActionProcessor(commandObj, activeApp = undefined,
                        wsMap)
                    ...
                    var activeApp = `${execSync("./src/Gens/Gen2/helper-scripts/activeApp.sh")}`
                        .split("=")[1]
                        .replace(", ", ".")
                        .replaceAll('"', "")
                        .trim();
